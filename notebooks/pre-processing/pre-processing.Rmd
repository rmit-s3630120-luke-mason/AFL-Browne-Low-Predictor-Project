...
## Feature Engineering
```{r}
# We want to incorporate the statistics of the game in comparison to
...
create_performance_dataset <- function(number_of_games, balanced = FALSE){
  ...
  game_stats <- all_stats[all_stats$gameId %in% unlist(gameIds),]
...
```{r}
performance_df = create_performance_dataset(100)
performance_df
```

### Create more balanced performance dataset
```{r}
balanced_performance_df = create_performance_dataset(1000, TRUE)
