...
# df = raw_stats %>% group_by(gameId)
top = all_stats %>% group_by(gameId) %>% summarise()

gameIds = as.list(head(top, 1))
game_stats_temp = all_stats[all_stats$gameId %in% unlist(gameIds),]
drops <- c("team",	"year",	"round", "displayName",	"gameNumber")
game_stats = game_stats_temp[ , !(names(game_stats_temp) %in% drops)]
...
for (id in gameIds) {
  ...
  for (row_index in 1:nrow(game)) {
    ...
    # Aggregate the rest of the player's stats
    player_agg = rowMeans(rest_of_players_filtered)
  
    # For each stat create a new
    row <- c()
    for (stat in stat_cols) {
      # print(stat)
      # print(rest_of_players)
      # Get the percentage difference between the player's stats and the rest of the player's
      percentage_difference <- player[stat][0] / player_agg[stat][0]
      diff = as.matrix(percentage_difference)
    ...
    append(row, as.matrix(player[xPlayed][0])/100)
    
    # Add label value to row
    append(row, player[label][0])
