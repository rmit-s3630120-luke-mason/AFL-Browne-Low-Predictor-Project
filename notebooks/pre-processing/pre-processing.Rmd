...
create_performance_dataset <- function(number_of_games, balanced = FALSE){
  ...
  drops <- c("team", "round", "displayName",	"gameNumber")
  ...
  # Column names.
  x <- c(stat_cols, c(xPlayed, label, "year"))
  ...
  for (id in unlist(gameIds)) {
    ...
    for (row_index in player_ids) {
      ...
      # Add year column to help splitting later
...
```{r}
balanced_performance_df = create_performance_dataset(10000, TRUE)
